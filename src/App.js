...
function App() {
  const onSubmit = (e) => {
    ...
    if (searchedvalue !== "") {
      let newText = text
        .toLowerCase()
        .replaceAll(searchedvalue, `<mark style="color:white;background-color:#17a2b8">${searchedvalue}</mark>`);
    ...
    <React.Fragment>
      ...
      <div style={{ border: "1px solid #000000", margin: "2rem 15rem" }}>
        <div className="input-group" style={{ width: "50rem", margin: "3rem 5rem" }}>
          <input type="text" className="form-control" placeholder="Enter text" name="searchText" onChange={onSubmit} />
        </div>
